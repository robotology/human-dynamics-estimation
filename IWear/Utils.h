...
namespace wear {
    namespace utils {
        ...
        wear::Quaternion normalizeQuaternion(const wear::Quaternion& quat)
        {
            double norm = std::sqrt(quat[0] * quat[0] + quat[1] * quat[1] + quat[2] * quat[2]
                                    + quat[3] * quat[3]);

            wear::Quaternion normalQuat(quat);

            if (norm != 1.0) {
                wear::Quaternion normalQuat;
        ...
        wear::Vector3 quaternionToRPY(const wear::Quaternion& quat)
        {
            wear::Quaternion q = wear::utils::normalizeQuaternion(quat);
        ...
        wear::Matrix3 quaterionToRotationMatrix(const wear::Quaternion& quat)
        {
            wear::Quaternion q = wear::utils::normalizeQuaternion(quat);
        ...
        wear::Quaternion rotationMatrixToQuaternion(const wear::Matrix3& rotMat)
        {
            ...
            return wear::utils::normalizeQuaternion(quat);
        };

        wear::Vector3 rotationMatrixToRPY(const wear::Matrix3& rotMat)
        {
        ...
        wear::Quaternion RPYToQuaternion(const wear::Vector3& rpy)
        {
        ...
        wear::Matrix3 RPYToRotationMatrix(const wear::Vector3& rpy)
        {
