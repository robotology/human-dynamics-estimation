...
bool RemappedSubControlBoard::attach(yarp::dev::PolyDriver *d, const std::string &k)
{
    ...
    if( !(pos2)  && (_subDevVerbose))
    {
        yWarning("ControlBoardRemapper: IPositionControl2 interface was not found in subdevice. Quitting");
    }

    if( ! (vel2)  && (_subDevVerbose))
    {
        yWarning("ControlBoardRemapper: IVelocityControl2 interface was not found in subdevice. Quitting");
    }

    if(!iJntEnc && (_subDevVerbose))
    {
        yWarning("ControlBoardRemapper: IEncoderTimed interface was not found in subdevice, exiting.");
    }

    if(!iMode2 && (_subDevVerbose))
    {
        yWarning("ControlBoardRemapper: IControlMode2 interface was not found in subdevice, exiting.");
    }
